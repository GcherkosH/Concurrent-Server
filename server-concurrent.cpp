/************************************************************ -- HEAD -{{{1- */
/* Concurrent Echo  Server for Network API 
 *
 *Concurrent server capable of accpeting and processing many connections
 * at any given time. Data received from the connection is simply sent back
 * unmodified ("echoed") to the client.
 *
 * Build the server using e.g.
 * 		$ g++ -Wall -Wextra -o server-concu server-concurrent.cpp
 *
 * Start using
 * 		$ ./server-concurrent
 * or
 * 		$ ./server-concurrent 31337
...
/* Places the socket identified by `filed' in non-blocking mode.
 *
 * Returns `true' if successful, and `false' otherwise.
 */
static bool set_socket_nonblocking( int filed );
 ...
 *
 * Returns, if successful, the new socket filed. On error, -1 is returned.
 */
...
#	endif
	...
	while( 1 )
	{
		...
		for(unsigned int i=0; i< connections.size(); i++){
			...
			fd_max = std::max(connections[i].sock,fd_max);	
		}	
...
static int setup_server_socket( short port )
{
	...
	if( -1 == bind( filed, (const sockaddr*)&servAddr, sizeof(servAddr) ) )
	{
		perror( "bind() failed" );
		close( filed );
		return -1;
	...
	if( -1 == getsockname( filed, (sockaddr*)&actualAddr, &actualAddrLen ) )
	{
	...
	if( -1 == listen( filed, kServerBacklog ) )
	{
	...
	int one = 1;
	if( -1 == setsockopt( filed, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(int) ) )
	{
...
#	if NONBLOCKING
	...
	if( !set_socket_nonblocking( filed ) )
	{
		close( filed );
		return -1;
	}
...
static bool set_socket_nonblocking( int filed )
{
	int oldFlags = fcntl( filed, F_GETFL, 0 );
	if( -1 == oldFlags )
	{
	...
	if( -1 == fcntl( filed, F_SETFL, oldFlags | O_NONBLOCK ) )
	{
